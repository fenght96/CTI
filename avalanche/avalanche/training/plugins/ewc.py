...
class EWCDetPlugin(EWCPlugin):
    ...
    def compute_importances(
        ...
        # dataloader = DataLoader(dataset, batch_size=batch_size)
        n_data = len(dataset)
        # n_data_list = [i for i in range(n_data)]
        # random.shuffle(n_data_list)
        # n_data_list = n_data_list[:1000]
        for i in range(0, n_data, batch_size):
            x,y = [], []
            for j in range(0, batch_size):
                if i + j >= n_data:
                    break
                x_,y_ = dataset[i+j][0], dataset[i+j][1]
                x.append(x_)
                y.append(y_)
            if len(x) <= 0:
                break
        ...
        # for i, batch in enumerate(dataset):
            # get only input, target and task_id from the batch
            # x, y, task_labels = batch[0].to(device), batch[1], batch[-1]
            
            if not isinstance(y, list):
                y = [y,]
            y = [{k: v.to(device) for k, v in t.items()}
                   for t in y]
            if not isinstance(x, list):
                x = [x,]
            # model.train()
            detection_loss_dict = model(x,y)
            # model.eval()
            
            optimizer.zero_grad()            
                
            loss_sum = sum(loss for k,loss in detection_loss_dict.items())
            loss_sum.backward()
