...
class EgoObjectsEval:
    def __init__(self, ego_gt, ego_dt, iou_type="segm"):
        ...
        self.params.img_ids = sorted(self.ego_gt.get_img_ids())
        self.params.cat_ids = sorted(self.ego_gt.get_cat_ids())

    def _to_mask(self, anns, ego):
        for ann in anns:
            rle = ego.ann_to_rle(ann)
    ...
    def _prepare(self):
        ...
        gts = self.ego_gt.load_anns(
            self.ego_gt.get_ann_ids(img_ids=self.params.img_ids, cat_ids=cat_ids)
        )
        dts = self.ego_dt.load_anns(
            self.ego_dt.get_ann_ids(img_ids=self.params.img_ids, cat_ids=cat_ids)
        )
        # convert ground truth to mask if iou_type == 'segm'
        if self.params.iou_type == "segm":
            self._to_mask(gts, self.ego_gt)
            self._to_mask(dts, self.ego_dt)
