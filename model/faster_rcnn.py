...
#     "FastRCNNBoxPredictor",
#     "FastRCNNsharePredictor",
#     "FastRCNNshareNormPredictor",
# ]


class FasterRCNNdemo(GeneralizedRCNN):
    """
    ...
    def __init__(
        ...
        min_size=900,#640 0.6478 800 0.7982 900 
        max_size=1504,
        image_mean=None,
        image_std=None,
        # RPN parameters
        ...
        transform = GeneralizedRCNNTransform(min_size, max_size, image_mean, image_std,mixup=mixup,device=device)
...
class FastRCNNBoxPredictor(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, embed_channels=2048):
        super().__init__()
        self.shared_fc = nn.Linear(in_channels, embed_channels)
        self.bbox_pred = nn.Linear(embed_channels, num_classes * 4)
...
class FastRCNNsharePredictor(nn.Module):
    """
    Standard classification + bounding box regression layers
    for Fast R-CNN.
    ...
    def __init__(self, in_channels, num_classes, embed_channels=2048):
        super().__init__()
        self.shared_fc = nn.Linear(in_channels, embed_channels)
        self.cls_score = nn.Linear(embed_channels, num_classes)
        self.bbox_pred = nn.Linear(embed_channels, num_classes * 4)
...
class FastRCNNshareNormPredictor(nn.Module):
    """
    Standard classification + bounding box regression layers
    for Fast R-CNN.
...
def fasterrcnn_resnet50_fpn(
    ...
    model = FasterRCNNdemo(backbone, num_classes, **kwargs)
    if pretrained:
