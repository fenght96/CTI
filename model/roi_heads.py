...
class DecorrelateLossClass(nn.Module):
    ...
    def __init__(self, reject_threshold=1, n_classes=277):
    ...
    def forward(self, x, y):
        ...
        for i, label in enumerate(uniq_l):
            if uniq_c[i] <= self.reject_threshold or label == self.n_classes:
                continue
            x_label = x[y==label, :]
...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets, loss_fn=None, features=None ):
    ...
    if isinstance(regression_targets, tuple):
    ...
    #     cls_loss = DecorrelateLossClass()
    #     classification_loss += 5 * cls_loss(features, labels)
    ...
    if loss_fn is None:
        ...
        box_loss = ciou_loss(
                box_regression[sampled_pos_inds_subset, labels_pos],
                regression_targets[sampled_pos_inds_subset].squeeze(1),
                reduction="sum",
            )
...
class RoIHeads(nn.Module):
    ...
    def forward(
        ...
        class_logits, box_regression, features_ = self.box_predictor(box_features)
        ...
        if self.training:
            ...
            if self.loss_fn is not None:
                ...
                regression_targets = self.box_coder.decode(regression_targets, proposals)
            loss_classifier, loss_box_reg = fastrcnn_loss(class_logits, box_regression, labels, regression_targets, self.loss_fn, features_)
